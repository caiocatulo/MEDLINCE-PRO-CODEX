...
public sealed class UnidadeHospitalarRepository : IUnidadeHospitalarRepository
{
    ...
    public async Task<IEnumerable<UnidadeHospitalar>> ListarPorEmpresaAsync(Guid empresaId, CancellationToken ct = default)
    {
        ...
        // Projeção preservada da consulta legada de Unidades Hospitalares.
        const string sql = @"
            SELECT
            ...
            FROM UnidadeHospitalar u
            INNER JOIN EmpresaUnidadeHospitalar eu ON u.CodigoCnes = eu.CodigoCnes
            WHERE eu.Ativo = 1
    ...
    public async Task<bool> AdicionarAsync(Guid empresaId, UnidadeHospitalar unidade, CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            const string sqlUpsertVinculo = @"
                ...
                ELSE
                    INSERT INTO dbo.EmpresaUnidadeHospitalar (EmpresaId, CodigoCnes, Ativo, DataCadastro)
                    VALUES (@EmpresaId, @CodigoCnes, 1, @DataCadastro);";

            var rows = await conn.ExecuteAsync(new CommandDefinition(
                sqlUpsertVinculo,
                new { EmpresaId = empresaId, unidade.CodigoCnes, DataCadastro = DateTime.UtcNow },
                transaction: tx,
